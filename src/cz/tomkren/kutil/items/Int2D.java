...
public class Int2D {
    ...
    public static Int2D mk(String str, String delimiter) {
        String[] parts = str.split(delimiter);
        return new Int2D(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]));
    }
    ...
    public int distance2(Int2D pos) {
        int dx = x - pos.x;
        int dy = y - pos.y;
        return dx*dx + dy*dy;
    }

    public Int2D direction(Int2D target) {
        return new Int2D(target.x - x, target.y - y);
    }
