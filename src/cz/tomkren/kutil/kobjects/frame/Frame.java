...
public class Frame extends KObject {
    ...
    public void init() {
        ...
        if (target.args().has("initCam")) {

            Log.it("BOND-HERE !!!");

            String initCamStr = target.args().getString("initCam");
            Int2D targetCamPos = Int2D.parseInt2D(initCamStr);

            cam.set(targetCamPos);
            computeCenter();
        }
    ...
    public void step() {
        ...
        if (simulationMaster().isSimulationRunning()) {
            for (KObject o : target.inside()) {
                if (o instanceof Player) {
                    ((Player) o).changeCam(this);
                }
            }
        }

        if (parent() instanceof Time) {

            if (drawingMaster().showGUI()) {
                ...
                if (jFrameZatimNezalozen) {
                    ...
                    selectionMaster().setSelectedFrame(this);
                }

                myJFrame.myJPanel.paintScreen(); // Zde začíná celé vykreslování ! :)
            }
        }
    }
    ...
    public void drag(Int2D clickPos, Int2D delta , Frame f) {
        if (isDecorHit(clickPos)) {
            ...
            target.dragInside(this, clickPos.minus(pos().plus(decorDelta)).minus(center), delta, zoom.get(), center); // #clickHell solved
        }
    }
    ...
    public void click(Int2D clickPos) {
        if (showXML.get() || isDecorHit(clickPos)) {
            ...
            target.clickInside(this, clickPos.minus(pos().plus(decorDelta)).minus(center), zoom.get(), center); // #clickHell solved
        }
    }
    ...
    public void wheel(Int2D wheelPos) {
        if (showXML.get() || isDecorHit(wheelPos)) {
            ...
            target.wheelInside(zoomCenter, zoom.get(), center); // #clickHell solved
        }
    }

    @Override
    public void release(Int2D clickPos, KObject obj) {
        target.releaseInside(clickPos.minus(pos().plus(decorDelta)).minus(center), zoom.get(), center); //(zoomCenter, zoom.get(), center); // #clickHell solved
    }


    /**
     * Vrací zda byla kliknutím zasažena vrchní lišta okna.
     */
    public boolean isDecorHit(Int2D clickPos) {
        return clickPos.minus( pos() ).getY() < decorY;
    }

    private Int2D getClickPos(MouseEvent e) {
        return new Int2D(e.getX(), e.getY()).minus(center);
    }

    private Int2D wheelPos(MouseWheelEvent e) {
        return getClickPos(e);
    }
    ...
    protected void mouseWheel(MouseWheelEvent e) {
        target.wheelInside(wheelPos(e), zoom.get(), center);  // #clickHell solved
        selectionMaster().getSelectedFrame().zoom(e);
        Log.it(selectionMaster().getSelectedFrame().zoomCenter);
    }

    protected void mouseClicked(MouseEvent e) {
        popupMaster().closePopupIfVisible();
        if (e.getButton() == MouseEvent.BUTTON3) {
            mouseMaster().clickedRightMouseButton(this, e.getX(), e.getY());
        }
        target.clickInside(this, getClickPos(e), zoom.get(), center); // #clickHell solved
    }

    protected void mouseReleased(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON3) {return;}
        target.releaseInside(getClickPos(e), zoom.get(), center); // #clickHell solved
    }
    ...
    protected void mouseDragged(MouseEvent e, Int2D delta) {
        ...
        target.dragInside(this, getClickPos(e), delta, zoom.get(), center); // #clickHell solved
        mouseMaster().onDrag(e);
        mouseMaster().onMove(e);
        if (e.getButton() == MouseEvent.BUTTON3) {
            mouseMaster().clickedRightMouseButton(this, e.getX(), e.getY());
        }
    }
